...
// MARK: - EmailMembersView (migrations 0028/0123/0124)
//
// Generalized "email these people" composer — the People-tab / chat analogue of
// CommitteeEmailComposer, but pool-agnostic: the caller hands in a resolved set
// of recipients (a house, the whole family, App Admins, a committee's roster,
// …). Everyone / By-Role / Pick, then a native Mail hand-off. Nothing is sent
// from the app.

/// Payload for the chat ⋯ "Email members" hand-off (#338) — presented as an
/// item-based sheet once the room's recipients have loaded.
struct ChatEmailData: Identifiable {
    let id = UUID()
    let title: String
...
struct EmailMembersView: View {
    @Environment(\.dismiss) private var dismiss

    let title: String
    let recipients: [EmailRecipient]
    /// Pre-select a role (e.g. opened from a committee's "Meals" channel).
    var presetArea: String? = nil
    ...
    private var areaOptions: [String] { Array(Set(recipients.flatMap(\.areas))).sorted() }

    private var audience: [EmailRecipient] {
        switch mode {
    ...
    private var audienceEmails: [String] { Array(Set(audience.map(\.email))).sorted() }

    private var shownForPick: [EmailRecipient] {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                if let presetArea, areaOptions.contains(presetArea) {
                    mode = .byRole; selectedArea = presetArea
                }
            }
            .sheet(isPresented: $showMail) {
                MailComposeView(recipients: audienceEmails, subject: subject).ignoresSafeArea()
            }
        }
    }
    ...
    private var content: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(spacing: 6) {
                        ForEach(areaOptions, id: \.self) { area in
                            ...
                            Button {
                                ...
                                HStack {
                                    Text(area).font(.mlrScaled(14, weight: .medium))
                ...
                if mode == .pick {
                    TextField("Search name or email…", text: $query).fieldStyle()
                    VStack(spacing: 0) {
                        ForEach(shownForPick) { r in
                            Button {
                                if selectedIds.contains(r.id) { selectedIds.remove(r.id) } else { selectedIds.insert(r.id) }
                            } label: {
                                HStack(spacing: 10) {
                    ...
                    .padding(.horizontal, 14).background(Color.mlrCard).clipShape(RoundedRectangle(cornerRadius: 12))
                }

                TextField("Subject (optional)", text: $subject).fieldStyle()

                Text(audienceLabel).font(.mlrCaption).foregroundStyle(Color.mlrTextMuted)

                Button {
                    if MailComposeView.canSend { showMail = true } else { copyAddresses() }
                } label: { Text("✉️ Open email").primaryButton() }
                .disabled(audienceEmails.isEmpty)
                .opacity(audienceEmails.isEmpty ? 0.5 : 1)

                Button { copyAddresses() } label: {
                    Text(copied ? "Copied ✓" : "Copy addresses")
                        .font(.mlrScaled(14, weight: .semibold)).foregroundStyle(Color.mlrPrimary)
                        .frame(maxWidth: .infinity).padding(.vertical, 12)
                        .background(Color.mlrPrimary.opacity(0.1)).clipShape(RoundedRectangle(cornerRadius: 12))
                }
                .disabled(audienceEmails.isEmpty)
            }
            .padding(20)
